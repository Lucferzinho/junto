...
export default function CriarTreino({ onCriado }: Props) {
  ...
  const labels = {
    ...
    pace: 'Pace estimado',
    tom: 'Tom do treino',
    desc: 'Descri\u00e7\u00e3o do percurso',
    iniciantes: 'Aberto a iniciantes',
    iniciantes2: 'Aparece com o selo no feed',
    btn: 'Publicar treino',
    salvando: 'Publicando...',
    sucesso: 'Treino publicado! Aparecendo no feed.',
  }
  ...
  return (
    <div style={{ padding:16, display:'flex', flexDirection:'column', gap:14, fontFamily:"'Barlow', sans-serif" }}>
      ...
      <div><label style={lbl}>{labels.nome}</label><input style={inp} placeholder="Ex: Long run de sabado" value={form.titulo} onChange={e => set('titulo', e.target.value)} /></div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
        <div><label style={lbl}>{labels.data}</label><input type="date" style={inp} value={form.data} onChange={e => set('data', e.target.value)} /></div>
        <div><label style={lbl}>{labels.horario}</label><input type="time" style={inp} value={form.horario} onChange={e => set('horario', e.target.value)} /></div>
      </div>

      <div><label style={lbl}>{labels.local}</label><input style={inp} placeholder="Ex: Aterro do Flamengo" value={form.local} onChange={e => set('local', e.target.value)} /></div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
        <div><label style={lbl}>{labels.km}</label><input type="number" min="1" style={inp} placeholder="18" value={form.km} onChange={e => set('km', e.target.value)} /></div>
        <div><label style={lbl}>{labels.max}</label><input type="number" min="2" style={inp} value={form.max} onChange={e => set('max', e.target.value)} /></div>
      </div>

      <div><label style={lbl}>{labels.tipo}</label><input style={inp} placeholder="Ex: Long run, Intervalado, Progressivo..." value={form.tipo} onChange={e => set('tipo', e.target.value)} /></div>

      <div><label style={lbl}>{labels.pace}</label><input style={inp} placeholder="Ex: 5:30/km, 5:00-6:00/km, A combinar..." value={form.pace} onChange={e => set('pace', e.target.value)} /></div>

      <div>
        <label style={lbl}>{labels.tom}</label>
          ...
          {TOMS.map(t => {
            ...
            return (
              <button key={t} onClick={() => set('tom', t)} style={{ padding:'6px 13px', borderRadius:20, border:`1px solid ${sel ? s.border : '#ddd'}`, background: sel ? s.bg : '#fff', color: sel ? s.color : '#666', fontSize:12, cursor:'pointer', fontFamily:"'Barlow', sans-serif", fontWeight: sel ? 700 : 500 }}>
                {t === 'Qualquer nivel' ? 'Qualquer n\u00edvel' : t}
              </button>
            )
          })}
        </div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 12px', border:'1px solid #ddd', borderRadius:8 }}>
        <div>
          <div style={{ fontSize:13, fontWeight:700, color:'#111' }}>{labels.iniciantes}</div>
          <div style={{ fontSize:12, color:'#888', marginTop:2 }}>{labels.iniciantes2}</div>
        </div>
      ...
      <div><label style={lbl}>{labels.desc}</label><textarea rows={3} style={{ ...inp, resize:'vertical' }} placeholder="Descreva o trajeto, pontos de parada..." value={form.descricao} onChange={e => set('descricao', e.target.value)} /></div>
      ...
      <button onClick={publicar} disabled={salvando} style={{ background: salvando ? '#ccc' : '#1D9E75', color:'#fff', border:'none', borderRadius:8, padding:13, fontSize:15, cursor: salvando ? 'not-allowed' : 'pointer', fontWeight:700, fontFamily:"'Barlow', sans-serif" }}>
        {salvando ? labels.salvando : labels.btn}
      </button>
    </div>
  )
}