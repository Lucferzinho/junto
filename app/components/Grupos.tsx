...
export default function Grupos() {
  ...
  const [membros, setMembros] = useState<Record<string, string>>({}) // grupo_id -> membro_id
  ...
  async function carregarGrupos() {
    ...
    const { data } = await supabase
      .from('grupos')
      .select('*, grupo_membros(id), usuarios(nome)')
  ...
  async function carregarMembros(uid: string) {
    ...
    if (data) {
      ...
      data.forEach(m => { map[m.grupo_id] = m.id })
      setMembros(map)
    }
  }
  ...
  async function abrirGrupo(grupo: Grupo) {
    ...
    const { data } = await supabase
      .from('grupo_membros')
      .select('usuario_id, usuarios(id, nome, avatar_url, pace_medio, nivel)')
  ...
  async function entrarGrupo(e: React.MouseEvent, grupoId: string) {
    ...
    setEntrando(grupoId)
    const { error, data: novo } = await supabase.from('grupo_membros').insert({ grupo_id: grupoId, usuario_id: userId }).select('id').single()
  ...
  async function criarGrupo() {
    if (!form.nome) { alert('Digite o nome do grupo!'); return }
    if (!userId) { alert('Faça login para criar um grupo!'); return }
    setSalvando(true)
    const { data: grupo, error } = await supabase.from('grupos').insert({ nome: form.nome, descricao: form.descricao, criador_id: userId }).select('id').single()
  ...
  const inp = { width: '100%', padding: '9px 12px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, outline: 'none', fontFamily: "'Barlow', sans-serif", marginBottom: 10 } as React.CSSProperties
    ...
    <div style={{ fontFamily: "'Barlow', sans-serif" }}>
      ...
      {criando && (
        <div style={{ padding: 16, borderBottom: '1px solid #eee', background: '#f9f9f9' }}>
          ...
          <input style={inp} placeholder="Nome do grupo *" value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} />
          <textarea rows={2} style={{ ...inp, resize: 'vertical' }} placeholder="Descrição (opcional)" value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))} />
        ...
        return (
          <div key={g.id} onClick={() => abrirGrupo(g)} style={{ padding: 16, borderBottom: '1px solid #eee', cursor: 'pointer' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 48, height: 48, borderRadius: 12, background: '#E1F5EE', color: '#085041', fontSize: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontWeight: 800 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {g.descricao && <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>{g.descricao}</div>}
